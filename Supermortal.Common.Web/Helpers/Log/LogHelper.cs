...
namespace Supermortal.Common.Web.Helpers.Log
{
                ...
                lock (Lock)
                {
                    ...
                    if (!Configured)
                    {
                        ...
                        if (fi.Exists)
                        {
                            //if (UsingEncryption)
                            //{
                            //    using (var textRdr = fi.OpenText())
                            //    {
                            //        var encryptedString = textRdr.ReadToEnd();
                            //        var decryptedBytes = Security.SymmetricDecrypt(encryptedString).ToStringFromStringBytesWithoutZeros().ToBytesWithoutZeros().ToArray();
